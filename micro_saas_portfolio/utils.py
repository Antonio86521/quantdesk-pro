...
def terminal_ribbon(items):
    ...
    html = textwrap.dedent(f"""
    <div style="display:flex; flex-wrap:wrap; gap:0; background:linear-gradient(180deg, #0b1220 0%, #0e1627 100%); border:1px solid {BORDER}; border-radius:7px; overflow:hidden; margin-bottom:12px;">
      {blocks}
    </div>
    """).strip()
