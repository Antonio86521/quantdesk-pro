...
GROUPS = {
    ...
    "Equities": ["S&P 500", "Nasdaq 100", "Dow Jones", "Russell 2000", "Euro Stoxx 50", "DAX", "FTSE 100", "Nikkei 225", "Hang Seng", "VIX"],
...
def load_macro_prices(labels, period_value="1y"):
    ...
    for label in labels:
        ...
        s = load_close_series(ticker, period=period_value)
        if not s.empty:
            df[label] = s
    return df.dropna(how="all")


def calc_snapshot(prices):
    rows = []
...
with st.spinner("Loading cross-asset market data…"):
    prices = load_macro_prices(active_labels, period)
...
headline_labels = [
    x for x in ["S&P 500", "Nasdaq 100", "VIX", "US 10Y", "DXY", "Gold", "WTI", "Bitcoin", "EUR/USD"]
...
c4.metric("Best 1D", best_1d["Asset"] if best_1d is not None else "—", delta=f"{best_1d['1D %']:.2f}%" if best_1d is not None else None)
c5.metric("Worst 1D", worst_1d["Asset"] if worst_1d is not None else "—", delta=f"{worst_1d['1D %']:.2f}%" if worst_1d is not None else None)
c6.metric("Best 3M", best_3m["Asset"] if best_3m is not None else "—", delta=f"{best_3m['3M %']:.2f}%" if best_3m is not None else None)
...
with tabs[0]:
    ...
    with left:
        section_header("Cross-Asset Monitor", "Sortable snapshot across the loaded universe")
    ...
    with right:
        section_header("1M Momentum Ranking", "Cross-asset relative strength")
        ...
        section_header("Performance Heatmap", "Fast cross-asset view")
        ...
        vmax = np.nanpercentile(np.abs(finite_vals), 95) if finite_vals.size > 0 else 1
        vmax = max(vmax, 0.25)
...
with tabs[1]:
    ...
    with col1:
        section_header("FX Dashboard", "Dollar, majors and relative performance")
        ...
        if not fx_prices.empty:
            selected_fx = st.multiselect("FX comparison", fx_prices.columns.tolist(), default=fx_prices.columns.tolist()[:4], key="fx_compare")
            if selected_fx:
                ...
                ax.set_title("FX Indexed to 100")
    ...
    with col2:
        section_header("Commodity Dashboard", "Precious metals, energy, industrials and grains")
        ...
        if not comm_prices.empty:
            selected_comm = st.multiselect("Commodity comparison", comm_prices.columns.tolist(), default=comm_prices.columns.tolist()[:4], key="comm_compare")
            if selected_comm:
                ...
                ax2.set_title("Commodities Indexed to 100")
...
with tabs[2]:
    section_header("Global Equities", "US and international index performance")
    ...
    if not eq_prices.empty:
        selected_eq = st.multiselect("Indices to chart", eq_prices.columns.tolist(), default=eq_prices.columns.tolist()[:5], key="eq_compare")
        if selected_eq:
            ...
            ax3.set_title("Equity Indices Indexed to 100")
            ax3.legend(fontsize=8, ncols=3)
...
with tabs[3]:
    ...
    with left:
        section_header("Rates", "Treasury yield proxies and curve signals")
        ...
        if set(["US 5Y", "US 10Y"]).issubset(rate_snap["Asset"].values):
            ...
            if set(["US 3M", "US 10Y"]).issubset(rate_snap["Asset"].values):
        ...
        if len(yc_labels) >= 2:
            ...
            x = [maturity_map[k] for k in yc_labels]
            y = [float(prices[k].dropna().iloc[-1]) for k in yc_labels]
            fig, ax = plt.subplots(figsize=(8, 4))
            ax.plot(x, y, marker="o", lw=2, color=ACCENT)
            ax.set_xticks(x)
            ax.set_xticklabels(yc_labels)
    ...
    with right:
        section_header("Bonds & Credit", "Treasury duration, IG, HY and inflation links")
        ...
        if set(["HYG", "LQD"]).issubset(bond_prices.columns):
            ...
            ax2.set_title("HYG / LQD Credit Risk Proxy")
            ax2.grid(True, alpha=0.2)
            plt.tight_layout()
            st.pyplot(fig2)
            plt.close()
...
with tabs[4]:
    section_header("Crypto Monitor", "Major crypto assets and volatility")
...
with tabs[5]:
    section_header("Correlation Matrix", "Daily return correlation across selected assets")
    ...
    if len(corr_assets) >= 2:
        ...
        if benchmark_label in prices.columns:
            section_header("Correlation vs Benchmark", benchmark_label)
            ...
            if vals:
                df = pd.DataFrame(vals, columns=["Asset", "Correlation"]).sort_values("Correlation")
                fig2, ax2 = plt.subplots(figsize=(9, max(4, len(df) * 0.3)))
                colors = [GREEN if v >= 0 else RED for v in df["Correlation"]]
                ax2.barh(df["Asset"], df["Correlation"], color=colors, edgecolor="#0a0e1a")
                ax2.axvline(0, color="white", lw=0.8, ls="--")
...
with tabs[6]:
    ...
    with left:
        section_header("Market Regime", "Simple trend + volatility overlay using S&P 500 and VIX")
    ...
    with right:
        section_header("Yield Curve Monitor", "Current curve shape and inversion risk")
        yc_labels = [x for x in ["US 3M", "US 5Y", "US 10Y", "US 30Y"] if x in prices.columns]
        if len(yc_labels) >= 2:
            maturity_map = {"US 3M": 0.25, "US 5Y": 5, "US 10Y": 10, "US 30Y": 30}
            ...
            if current is not None:
                ...
                if "US 10Y" in current.index and "US 3M" in current.index:
                    ...
                    st.metric("10Y - 3M Spread", f"{spread:.2f}", delta="Inverted" if spread < 0 else "Normal")
