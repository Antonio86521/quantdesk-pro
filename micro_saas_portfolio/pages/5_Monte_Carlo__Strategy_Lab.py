...
from strategies import (
    ...
    payoff_long_put, payoff_short_put,
...
mc_paths_n   = st.sidebar.slider("Number of paths", 100, 10000, 3000, 100)
...
if run_mc:
    ...
    call_err = abs(mc_call_payoff.mean() - bs_call) / bs_call * 100
    put_err  = abs(mc_put_payoff.mean()  - bs_put)  / bs_put  * 100
    st.caption(
        f"MC error vs BS — Call: {call_err:.2f}%  |  Put: {put_err:.2f}%  "
        f"({mc_paths_n:,} paths, antithetic variates)"
    )
    ...
    for i in range(n_show):
        ax.plot(t_axis, paths[:, i], lw=0.4, alpha=0.2, color=ACCENT)
    ...
    ax.axhline(option_K, color=YELLOW, ls=":", lw=1.5, label=f"Strike ${option_K:.0f}")
    ax.set_title(f"Monte Carlo — {mc_paths_n:,} paths over {mc_horizon:.1f} yr(s)")
    ...
    ax2.hist(final_prices, bins=70, color=ACCENT2, alpha=0.75, edgecolor="#0a0e1a")
    ax2.axvline(option_K,                  color=YELLOW, lw=1.5, ls="--", label=f"Strike ${option_K:.0f}")
    ...
    prob_above = (final_prices > option_K).mean() * 100
    s6.metric("P(above K)",  f"{prob_above:.1f}%")

    # ── MC convergence
    st.markdown("#### MC Convergence — Price vs Paths")
    sample_sizes = np.unique(np.linspace(100, mc_paths_n, 40).astype(int))
    conv_prices  = []
    for n in sample_sizes:
        payoff = np.maximum(paths[-1, :n] - option_K, 0)
        conv_prices.append(np.exp(-r_mc * mc_horizon) * payoff.mean())

    fig3, ax3 = plt.subplots(figsize=(10, 3))
    ax3.plot(sample_sizes, conv_prices, color=ACCENT, lw=1.5)
    ax3.axhline(bs_call, color=YELLOW, lw=1.2, ls="--", label=f"BS Price ${bs_call:.4f}")
    ax3.set_xlabel("Number of Paths"); ax3.set_ylabel("Call Price ($)")
    ax3.set_title("MC Convergence to BS Price")
...
if pnl is not None:
    ...
    ax_s.fill_between(S_range, pnl, 0,
                      where=pnl >= 0, alpha=0.15, color=GREEN, label="Profit zone")
    ax_s.fill_between(S_range, pnl, 0,
                      where=pnl <  0, alpha=0.15, color=RED,   label="Loss zone")
    ax_s.axhline(0, color="white", ls="--", lw=1)
    ax_s.axvline(option_S, color=GREEN,  ls="--", lw=1, label=f"Spot ${option_S:.0f}")
    ax_s.axvline(option_K, color=YELLOW, ls="--", lw=1, label=f"Strike ${option_K:.0f}")

    for be in summary["breakevens"]:
        ax_s.axvline(be, color="#a78bfa", ls=":", lw=1.2)
        ax_s.annotate(f"B/E\n${be:.1f}", (be, ax_s.get_ylim()[0]),
                      fontsize=7, color="#a78bfa", ha="center", va="bottom")

    ax_s.set_xlabel("Stock Price at Expiry ($)")
    ax_s.set_ylabel("P&L ($)")
    ...
    m1.metric("Max Profit",  f"${summary['max_profit']:,.2f}"
              if not np.isinf(summary['max_profit']) else "Unlimited")
    m2.metric("Max Loss",    f"${summary['max_loss']:,.2f}"
              if not np.isinf(summary['max_loss']) else "Unlimited")
    if summary["breakevens"]:
        m3.metric("Breakeven(s)", ", ".join([f"${b:.1f}" for b in summary["breakevens"]]))
    else:
        m3.metric("Breakeven(s)", "None in range")
